...
def test_ponytail_debt_ledger():
    ...
    for item in list_items:
        ...
        assert " — " in item or " \u2014 " in item, f"Entry missing long dash: {item}"
        assert ". ceiling: " in item, f"Entry missing '. ceiling: ': {item}"
        assert ". upgrade: " in item, f"Entry missing '. upgrade: ': {item}"
        assert item.endswith(".") or item.endswith("[no-trigger]"), f"Entry does not end with expected ending: {item}"
