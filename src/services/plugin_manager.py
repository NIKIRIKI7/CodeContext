...
class IPlugin:
    ...
    name: str = ""
    version: str = ""
    def on_init(self, main_controller) -> None: pass
...
class PluginManager:
    ...
    def load_plugin(self, manifest: dict):
        ...
        try:
                ...
                for filename in os.listdir(locales_dir):
                    if filename.endswith(".json"):
                        ...
                        with open(os.path.join(locales_dir, filename), "r", encoding="utf-8-sig") as f:
                            add_plugin_translations(lang_code, json.load(f))
            ...
            # ponytail: Песочницы не работают в Python. Отдаем контроллер плагинам напрямую.
            plugin_instance.on_init(self.container.main_controller)
