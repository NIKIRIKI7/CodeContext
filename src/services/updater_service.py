...
def _download_sync(url: str, target: str, cb: Callable[[float], None]) -> bool:
    global target_app
    sys_os = platform.system()
    is_mac = (sys_os == "Darwin")

    if is_mac:
        target_app = _get_mac_app_path(target)
...
def _parse_version(v_str: str) -> Tuple[int, int, int, int, int]:
...
async def download_and_install(download_url: str, progress_cb: Callable[[float], None]) -> bool:
